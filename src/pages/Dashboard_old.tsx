...
const Dashboard: React.FC = () => {
  ...
  const checkKeyStatus = async (gameName: string) => {
    try {
      ...
      const response = await fetch(`/api/dashboard/logs?key=${userKey}${gameName ? `&game=${gameName}` : ''}`, {
        headers: {
      ...
      if (gameName) {
        ...
        // Update game data with new API response structure
        if (data.data && data.data.hasData) {
          const playersData = data.data.playersData || {};
          const playerNames = Object.keys(playersData);
          
          setGameData({
            accounts: playersData,
            stats: {
              activeAccounts: data.data.players || 0,
              totalAccounts: playerNames.length,
              totalMoney: data.data.stats?.totalMoney || 0,
              avgLevel: data.data.stats?.avgLevel || 0,
              totalPlayers: data.data.stats?.totalPlayers || 0
            }
          });
          
          // Extract items from accounts
          if (playersData) {
            extractItems(playersData);
          }
        }
      } else {
        // All games status
  ...
  const extractItems = (accounts: Record<string, any>) => {
    ...
    Object.values(accounts).forEach((account: any) => {
      // Extract from new script structure (arrays of objects with name, image, sprite)
      ['melee', 'sword', 'gun', 'devil_fruit', 'fruit_inventory', 'accessories'].forEach(category => {
        if (Array.isArray(account[category])) {
          account[category].forEach((item: any) => {
            ...
            if (!itemCounts[key].owners.includes(account.username || Object.keys(accounts)[0])) {
              itemCounts[key].owners.push(account.username || Object.keys(accounts)[0]);
            }
          });
        }
      });
      
      // Extract from individual items (backward compatibility)
