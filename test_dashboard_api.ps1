...
try {
    $url = "http://localhost:3001/api/dashboard/logs?key=$dashboardKey&game=$gameName"
...
try {
    $response = Invoke-RestMethod -Uri "http://localhost:3001/api/dashboard/update" -Method Post -Body ($testData | ConvertTo-Json -Depth 10) -ContentType "application/json"
