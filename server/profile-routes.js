...
router.put('/:userId', verifyToken, upload.single('avatar'), (req, res) => {
  try {
    const { userId } = req.params;
    ...
    if (req.user.id !== userId) {
      return res.status(403).json({
        success: false,
        message: 'ไม่สามารถแก้ไขโปรไฟล์ของผู้อื่น'
      });
    }
    ...
    if (avatarFile) {
      ...
      // Set new avatar path
      data.users[userIndex].avatar = `/uploads/avatars/avatar-${userId}.jpg`;
    }
