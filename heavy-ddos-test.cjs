/**
 * Heavy DDoS Test - Test Protection Limits
 * ทดสอบ DDoS หนักๆ เพื่อดูขีดจำกัดของระบบ
 */

const http = require('http');

async function heavyDDoSTest() {
  console.log('🚀 Starting HEAVY DDoS Test...');
  console.log('📍 Target: http://localhost:3001/api/health');
  console.log('⚡  Sending 500 rapid requests...\n');
  ...
  // Results
  console.log('📊 Heavy Test Results:');
  console.log(`⏱️  Total Time: ${duration}ms`);
  console.log(`🚀 Requests/Second: ${(500 / duration * 1000).toFixed(2)}`);
  ...
  const protectionRate = (totalProtected / 500) * 100;
  ...
  console.log('\n💡 Next Steps:');
  if (protectionRate === 0) {
    console.log('   - Try sending 1000+ requests');
    console.log('   - Check server logs for security events');
    console.log('   - Verify security middleware is properly configured');
  } else {
    console.log('   - Protection is working at high load');
    console.log('   - Monitor server performance during attacks');
    console.log('   - Check for false positives');
  }
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'User-Agent': `Heavy-DDoS-Test-${requestId}`,
        'X-Test-Request': 'true',
        'X-Load-Test': 'heavy'
      }
    };
...
// Run the test
heavyDDoSTest().catch(console.error);
