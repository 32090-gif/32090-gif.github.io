-- ============================================
-- Kunlun Dashboard Script (Update Existing Data)
-- ============================================
...
-- ⚙️ Settings
-- ============================================
local AUTO_UPDATE_INTERVAL = 5
local MONEY_CHECK_INTERVAL = 5
...
local function sendToWebServer(data)
    local success, response = pcall(function()
        return HttpService:RequestAsync({
            Url = WEB_SERVER_URL .. WEB_API_ENDPOINT,
            Method = "POST",
            Headers = {
    ...
    if success and response then
        ...
        local successDecode, responseData = pcall(function()
            return HttpService.JSONDecode(HttpService, response.Body)
        end)
...
-- ============================================
-- 📤 Main Update Function (Update Existing Data)
-- ============================================
local function sendUpdate(customData)
    local account = customData or getAccountData()
    
    -- ส่งข้อมูลไปยัง Web Server API (จะทับข้อมูลเก่า)
    sendToWebServer(account)
    
    print("📊 อัปเดตข้อมูลใน Database สำเร็จ")
end
...
-- ส่งข้อมูลครั้งแรก
sendUpdate()
sendNotification("Kunlun Dashboard", "Database Update Mode", 2.5)
...
print("📊 Update Mode - ส่งข้อมูลไปอัปเดตใน Database")
...
print("4. Script จะส่งข้อมูลไปอัปเดตใน Database ทุก 5 วินาที")
print("5. ข้อมูลเก่าจะถูกทับด้วยข้อมูลใหม่")
print("")
print("🔍 Debug Info:")
